...
`pipeline/protection.py` masks numeric values and units before translation and judge calls. Translation output must preserve every sentinel and occurrence count. Missing/changed/extra sentinels and invented numbers return a blocked, reviewable draft with stage-labeled failure details. Only exact known markers are restored for display; missing values are not inserted and malformed markers stay visibly unresolved. Later stages stop, and no model safety pass is claimed. Fresh edit checks compare source numeric occurrence counts before clearing failures on a new revision. Judge outages, incomplete audit objects, and invalid verdicts become `FLAGGED_FOR_REVIEW`.

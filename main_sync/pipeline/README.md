...
`pipeline/protection.py` masks clinical values: doses/concentrations, temperatures, phone numbers including 911, timing/frequency, percentages, and clinical measurements. It does not mask age, list numbering, dates, protocol versions, or every bare number. Output must preserve every distinct sentinel at least once; repetition counts need not match. Missing/changed/extra sentinels and invented protected values return blocked, reviewable drafts with stage-labeled details. When Spanish is requested, translation and back-translation still run after failures, and the judge always runs; deterministic failures continue to override a model `PASS`. `pipeline/text_formatting.py` removes Markdown presentation syntax from simplified English before evaluation and display while preserving text structure.

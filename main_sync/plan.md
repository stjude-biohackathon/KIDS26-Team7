...
- **Component 1 (Vetted English Composer)**:
  - Binds supplied versioned wording and appends every populated instruction-bearing field from the immutable Team7 clinical order, then instructs LLM1: “Change the language, not the information. Preserve every instruction, fact, condition, exception, warning, and clinical value.” Patient demographics such as name/MRN, age, and weight remain in the source record and UI but are omitted from simplification context. The UI sends the loaded Team7 order unchanged; physicians edit only the generated simplified instructions. LLM1 targets actual restored-text FKGL 5.0–6.9 and retries from the source with score feedback up to three times. The last readable model response remains visible even when FKGL is out of range. Clinician edits are rechecked and must meet the same approval gates.
...
- **Component 1 (Canonical Contract)**:
  - `schemas/instruction_packet.py`: `InstructionPacket`, `ClinicalOrders`, `MedicationOrder`, `EvaluationMetrics`. `ClinicalOrders` carries weight, hydration, red flags, contraindications, thresholds, contacts, medications, and source version metadata. `InstructionPacket` records independent source and generation snapshots of the same immutable Team7 order for auditability; the UI provides no order override path.
...
- **Component 1 (Sidebar Controls)**:
  ...
  - Section 2 is a compact GitHub App connection indicator with a green/red state and adjacent Refresh button. Protocol/version and drift controls are sections 3–4. The redundant active-protocol summary is omitted. Team7 clinical orders have no sidebar fields, medication controls, or override path; the selected order is passed unchanged into generation. Physicians make changes only in the simplified-instructions editor after generation. Generation and source preview are blocked unless the reported source is the Team7 GitHub App; bundled mock/fallback content is never presented as original clinical instructions.
- **Component 2 (Source Preview & Comparative Review)**:
  - Front page: title `Pediatric Discharge Instruction Review`; two lines showing prototype name John Doe, active MRN/age, and sex M; a larger immediately updated module name; centered Generate Simplified Instructions and unchecked `Generate Spanish` controls; then a full-width scrollable source preview. The preview styles the immutable Team7 wording and source order with clear section headings, labeled fields, indented medication cards, and readable spacing without changing or omitting clinical statements. Empty source fields are omitted instead of filled with clinical defaults. Streamlit's top status/loading decoration is hidden. An in-page generation spinner appears directly beneath the page heading while the source preview remains visible, and the page switches to comparative review only after the simplified packet is ready.
  - After generation: original and editable simplified English start at equal width and height. The original pane uses the same immutable Team7-backed source renderer as the pre-generation preview; generation and safety checks use an independent unchanged copy of that order. The original pane is horizontally resizable and gives remaining width to the editor; its visible field label is collapsed. Spanish and back-translated English appear in a second equal-width row when requested. There is no viewport slider. New Generation returns to the preview.

...
- **Component 1 (Sidebar Controls)**:
  ...
  - Section 2 is a compact GitHub App connection indicator with a green/red state and adjacent Refresh button. Protocol/version, clinical-order, and drift controls are sections 3–5. A load failure blocks generation.
- **Component 2 (Source Preview & Comparative Review)**:
  - Front page: title `Pediatric Discharge Instruction Review`; two lines showing prototype name John Doe, active MRN/age, and sex M; a larger immediately updated module name; Generate Simplified Instructions with an unchecked `Generate Spanish` checkbox; then a full-width scrollable source box.
...
Verified on this iteration: **160 automated tests passed with 116 subtests**, including Python syntax exercised through imports. The suite uses synthetic fixtures and mocked external transports, blocks network requests/user-secret reads, and prohibits JSONL file access. It covers all three conditions and review outcomes, revision freshness, session isolation, clinically scoped protected values, continued translation/back-translation after failed checks, plain-text editor formatting, optional English-only publication, visible failed drafts, repaired-revision checks, blocked publishing/export, sequential Spanish-first bilingual PDF exports, the resizable review layout, compact GitHub status, and optional rejection metadata.

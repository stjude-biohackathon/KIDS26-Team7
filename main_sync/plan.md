...
- **Component 1 (Vetted English Composer)**:
  - Binds supplied versioned wording and appends every populated instruction-bearing field from the effective clinical order, then instructs LLM1: “Change the language, not the information. Preserve every instruction, fact, condition, exception, warning, and clinical value.” Patient demographics such as name/MRN, age, and weight remain in the source record and UI but are omitted from simplification context. The effective order starts as an exact copy of the Team7 order and may contain explicit physician overrides. It targets actual restored-text FKGL 5.0–6.9 and retries from the source with score feedback up to three times. The last readable model response remains visible even when FKGL is out of range. Clinician edits are rechecked and must meet the same approval gates.
...
- **Component 3 (Safety Judge - LLM2)**:
  - Compares readable simplified English with the single composed original-clinical-instructions source used by LLM1; it receives neither a separate structured-orders checklist nor protected-marker tags. It uses holistic semantic judgment and prefers `PASS` when a reasonable clinician would find the same usable information despite plain-language rewording, sentence splitting/combining, formatting changes, omitted labels/headings/tags, details folded into prose, or removal of exact repetition. Deterministic checks separately enforce protected values. The judge still detects definite factual drift, red-flag omission, changed conditions or urgency, added clinical advice, and semantic contradictions.
...
- **Component 5 (Scenario C Drift Simulator)**:
  ...
  - Simplification and translation inputs mask instruction-level clinical values with sentinels: doses/concentrations, temperatures, phone numbers including 911, timing/frequency, percentages, and non-demographic clinical measurements. Patient name/MRN, age, and weight are not protected values and their absence from simplified instructions does not fail a check; these demographics are omitted from structured model context. List numbering, dates, and protocol versions are also not protected. Restoration requires each distinct protected marker at least once and rejects missing/altered/unknown markers and invented protected values; differing repetition counts alone do not fail approval. Preserve failed drafts and attach stage-labeled `protection_failures`. When Spanish is requested, translation and back-translation still run after earlier check failures, followed by the safety judge using restored readable text. All findings remain approval blocks.

### Track B: In-Memory Data Loader, Storage & PDF Exporter (Participant 2)
- **Component 1 (Canonical Contract)**:
  - `schemas/instruction_packet.py`: `InstructionPacket`, `ClinicalOrders`, `MedicationOrder`, `EvaluationMetrics`. `ClinicalOrders` carries weight, hydration, red flags, contraindications, thresholds, contacts, medications, and source version metadata. `InstructionPacket` stores the immutable Team7 source order separately from the effective order used for generation.
- **Component 2 (In-Memory GitHub App Loader)**:
  ...
  - Team7 module `instruction_text` and synthetic-order values are the only clinical source for the original instructions. The adapter preserves every instruction exactly, rejects the entire load when any instruction record is incomplete, and maps weight, medication details, hydration, thresholds, red flags, contraindications, contacts, and version metadata into the canonical order. Absent order values remain blank instead of receiving schema or UI demo defaults. Order-specific text remains in the structured order rather than being appended to and confused with module wording.
- **Component 3 (Versioned Library Persistence)**:
  - `ReviewLibrary` stores copied, reviewed snapshots in session memory. Reads return independent copies. Repeated identical saves are idempotent; conflicting records with the same ID are rejected. Pending packets are not saved; rejection category and rationale are optional.
...
- **Component 4 (Bilingual PDF Generator)**:
  - ReportLab creates a full-width English handout by default. When Spanish is included, it prints a full-width Spanish section first and the simplified English source on the next page. PDFs include review badges, bold protected values, versions/timestamps, and page numbers; long paragraphs split safely across pages. Rejected PDFs are marked audit-only, not for patient use. Supplied text and metadata are escaped.
...
- **Component 1 (Sidebar Controls)**:
  ...
  - Section 2 is a compact GitHub App connection indicator with a green/red state and adjacent Refresh button. Protocol/version, clinical-order, and drift controls are sections 3–5. The redundant active-protocol summary is omitted. The clinical-order fields are editable physician overrides initialized directly from Team7. Clinicians can change all loaded order fields and append blank medication rows. Generation and source preview are blocked unless the reported source is the Team7 GitHub App; bundled mock/fallback content is never presented as original clinical instructions.
- **Component 2 (Source Preview & Comparative Review)**:
  - Front page: title `Pediatric Discharge Instruction Review`; two lines showing prototype name John Doe, active MRN/age, and sex M; a larger immediately updated module name; centered Generate Simplified Instructions and unchecked `Generate Spanish` controls; then a full-width scrollable source preview. The preview styles the immutable Team7 wording and source order with clear section headings, labeled fields, indented medication cards, and readable spacing without changing or omitting clinical statements. Empty source fields are omitted instead of filled with clinical defaults. If a sidebar value differs, the preview names the physician-overridden fields while continuing to show the source values. Streamlit's top status/loading decoration is hidden. An in-page generation spinner appears directly beneath the page heading while the source preview remains visible, and the page switches to comparative review only after the simplified packet is ready.
  - After generation: original and editable simplified English start at equal width and height. The original pane uses the same immutable Team7-backed source renderer as the pre-generation preview; generation and safety checks use the separately recorded effective order containing physician overrides. The original pane is horizontally resizable and gives remaining width to the editor; its visible field label is collapsed. Spanish and back-translated English appear in a second equal-width row when requested. There is no viewport slider. New Generation returns to the preview.
...
- **Component 4 (Action Gates & Governance)**:
  ...
  - "Reject & log drift": Persistent modal with Cancel and optional category/reason. Rejects a copy of the current reviewed text, builds its audit PDF before saving, and preserves previous history on failure. Finalized packets require a new revision before a changed review.
  - The concise library viewer displays only Module, Status, Physician Decision, and Timestamp.
...
Verified on this iteration: **169 automated tests passed with 116 subtests**, including Python syntax exercised through imports. The suite uses synthetic fixtures and mocked external transports, blocks network requests/user-secret reads, and prohibits JSONL file access. It covers all three conditions and review outcomes, revision freshness, session isolation, clinically scoped protected values, continued translation/back-translation after failed checks, plain-text editor formatting, optional English-only publication, visible failed drafts, repaired-revision checks, blocked publishing/export, sequential Spanish-first bilingual PDF exports, the resizable review layout, exact Team7 source-instruction preservation including the complete 100.4°F action statement, rejection of incomplete/non-GitHub source content, complete Team7 order mapping, absence of invented remote order defaults, immutable source display alongside physician overrides, formatted source preview, centered generation controls, medication addition, concise library records, compact GitHub status, and optional rejection metadata.

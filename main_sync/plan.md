...
- **Component 3 (Inline Editing & Feedback Loop)**:
  ...
  - Readability, protected-value, and judge failures retain the simplified draft and any already-produced panes. The UI shows a not-for-patient-use reason; protected failures also list failed tokens and display FAILED instead of a misleading passing percentage. Failed drafts have no approval eligibility. Rechecks require every distinct source numeric value at least once; repetition counts may differ, while missing or changed instructions still fail semantic review. A new passing revision is required to publish.

...
- **Component 2 (In-Memory GitHub App Loader)**:
  ...
  - Team7 module `instruction_text` and synthetic-order values are the only clinical source for the original instructions. The adapter preserves every usable instruction exactly, carries source hydration/red-flag text forward, rejects incomplete instruction records, and leaves absent order values blank instead of applying schema or UI demo defaults.
...
- **Component 1 (Sidebar Controls)**:
  ...
  - Section 2 is a compact GitHub App connection indicator with a green/red state and adjacent Refresh button. Protocol/version, clinical-order, and drift controls are sections 3–5. The redundant active-protocol summary is omitted. Clinicians can append blank medication rows in the medication panel. Generation and source preview are blocked unless the reported source is the Team7 GitHub App; bundled mock/fallback content is never presented as original clinical instructions.
- **Component 2 (Source Preview & Comparative Review)**:
  - Front page: title `Pediatric Discharge Instruction Review`; two lines showing prototype name John Doe, active MRN/age, and sex M; a larger immediately updated module name; centered Generate Simplified Instructions and unchecked `Generate Spanish` controls; then a full-width scrollable source preview. The preview styles the Team7 wording with clear section headings, labeled fields, indented medication cards, and readable spacing without changing or omitting clinical statements. Empty source fields are omitted instead of filled with clinical defaults. Streamlit's top status/loading decoration is hidden. An in-page generation spinner appears while the source preview remains visible, and the page switches to comparative review only after the simplified packet is ready.
  - After generation: original and editable simplified English start at equal width and height. The original pane uses the same Team7-backed source renderer as the pre-generation preview, is horizontally resizable, and gives remaining width to the editor; its visible field label is collapsed. Spanish and back-translated English appear in a second equal-width row when requested. There is no viewport slider. New Generation returns to the preview.
...
Verified on this iteration: **166 automated tests passed with 116 subtests**, including Python syntax exercised through imports. The suite uses synthetic fixtures and mocked external transports, blocks network requests/user-secret reads, and prohibits JSONL file access. It covers all three conditions and review outcomes, revision freshness, session isolation, clinically scoped protected values, continued translation/back-translation after failed checks, plain-text editor formatting, optional English-only publication, visible failed drafts, repaired-revision checks, blocked publishing/export, sequential Spanish-first bilingual PDF exports, the resizable review layout, exact Team7 source-instruction preservation, rejection of incomplete/non-GitHub source content, absence of invented remote order defaults, formatted source preview, centered generation controls, medication addition, concise library records, compact GitHub status, and optional rejection metadata.

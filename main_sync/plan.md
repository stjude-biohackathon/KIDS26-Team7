...
- **Component 1 (Sidebar Controls)**:
  ...
  - Section 2 is a compact GitHub App connection indicator with a green/red state and adjacent Refresh button. Protocol/version, clinical-order, and drift controls are sections 3–5. The redundant active-protocol summary is omitted. Clinicians can append blank medication rows in the medication panel. A load failure blocks generation.
- **Component 2 (Source Preview & Comparative Review)**:
  - Front page: title `Pediatric Discharge Instruction Review`; two lines showing prototype name John Doe, active MRN/age, and sex M; a larger immediately updated module name; centered Generate Simplified Instructions and unchecked `Generate Spanish` controls; then a full-width scrollable source preview. The preview uses clear section headings, labeled fields, indented medication cards, and readable spacing instead of raw delimiter headings. Streamlit's top status/loading decoration and generation spinner are hidden.
  - After generation: original and editable simplified English start at equal width and height. The original pane is horizontally resizable and the editor receives the remaining width; its visible field label is collapsed. Spanish and back-translated English appear in a second equal-width row when requested. There is no viewport slider. New Generation returns to the preview.
...
Verified on this iteration: **162 automated tests passed with 116 subtests**, including Python syntax exercised through imports. The suite uses synthetic fixtures and mocked external transports, blocks network requests/user-secret reads, and prohibits JSONL file access. It covers all three conditions and review outcomes, revision freshness, session isolation, clinically scoped protected values, continued translation/back-translation after failed checks, plain-text editor formatting, optional English-only publication, visible failed drafts, repaired-revision checks, blocked publishing/export, sequential Spanish-first bilingual PDF exports, the resizable review layout, formatted source preview, centered generation controls, medication addition, concise library records, compact GitHub status, and optional rejection metadata.

...
- **Component 1 (Vetted English Composer)**:
  - Binds supplied versioned wording and orders, then LLM1 only simplifies existing instructions: replace difficult words and split long sentences; retain already-simple wording, original order, all actions, conditions, exceptions, and urgency. Do not rewrite into a new handout, summarize, reorganize, add advice/explanations, or drop instructions. Protected values remain associated with the correct instructions. Measure actual restored-text FKGL; retry from the source with score feedback up to three attempts to reach 5.0–6.9. Failure stops generation before translation. Clinician edits are rechecked, never silently rewritten, and must meet the same approval benchmark.
...
- **Component 5 (Scenario C Drift Simulator)**:
  ...
  - Model inputs mask numeric values and units with sentinels; restoration requires each distinct protected marker at least once and rejects missing/altered/unknown markers and invented numeric values; differing repetition counts alone do not fail approval. Preserve the failed model draft for clinician review, restore only intact known markers, and attach stage-labeled `protection_failures` to the evaluation. Do not guess missing values or continue later model stages after failure. Incomplete/malformed judge audits are flagged. New-value and FKGL checks can veto a passing model judge. Warning meaning is checked by the judge and clinician, allowing legitimate rephrasing; reported omissions block approval even with a PASS verdict.
...
- **Component 3 (Inline Editing & Feedback Loop)**:
  ...
  - Protected-value failures retain available draft panes, show a not-for-patient-use banner and failed-token list, and display FAILED instead of a misleading passing verbatim percentage. These drafts have no successful-check snapshot. Rechecks require every distinct source numeric value at least once, including source-only values; repetition counts may differ, but missing or changed instructions still fail semantic review; a new passing revision is required to clear failures.
...
Verified on this iteration: **149 automated tests passed**, Python syntax checks passed, and `git diff --check` was clean. The suite uses synthetic fixtures and mocked external transports, blocks network requests/user-secret reads, and prohibits JSONL file access. It covers all three conditions and review outcomes, revision freshness, session isolation, drift injections, protected translation, optional English-only generation/rechecking/publication, input freshness, reviewable protected-value failures at each model stage, repaired-revision checks, blocked publishing/export of failed drafts, and long English/bilingual PDF exports.

...
- **Component 1 (Vetted English Composer)**:
  - Binds supplied versioned wording and appends every populated field from the effective clinical order, then instructs LLM1: “Change the language, not the information. Preserve every instruction, fact, condition, exception, warning, and clinical value.” The effective order starts as an exact copy of the Team7 order and may contain explicit physician overrides. It targets actual restored-text FKGL 5.0–6.9 and retries from the source with score feedback up to three times. The last readable model response remains visible even when FKGL is out of range. Clinician edits are rechecked and must meet the same approval gates.
...
- **Component 2 (Source Preview & Comparative Review)**:
  - Front page: title `Pediatric Discharge Instruction Review`; two lines showing prototype name John Doe, active MRN/age, and sex M; a larger immediately updated module name; centered Generate Simplified Instructions and unchecked `Generate Spanish` controls; then a full-width scrollable source preview. The preview styles the immutable Team7 wording and source order with clear section headings, labeled fields, indented medication cards, and readable spacing without changing or omitting clinical statements. Empty source fields are omitted instead of filled with clinical defaults. If a sidebar value differs, the preview names the physician-overridden fields while continuing to show the source values. Streamlit's top status/loading decoration is hidden. An in-page generation spinner appears while the source preview remains visible, and the page switches to comparative review only after the simplified packet is ready.

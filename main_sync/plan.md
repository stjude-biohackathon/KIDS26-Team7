...
- **Component 5 (Scenario C Drift Simulator)**:
  ...
  - Model inputs mask numeric values and units with sentinels; restoration rejects missing, duplicated, unknown markers and invented numeric values as approval failures. Preserve the failed model draft for clinician review, restore only intact known markers, and attach stage-labeled `protection_failures` to the evaluation. Do not guess missing values or continue later model stages after failure. Incomplete/malformed judge audits are flagged. New-value and FKGL checks can veto a passing model judge. Warning meaning is checked by the judge and clinician, allowing legitimate rephrasing; reported omissions block approval even with a PASS verdict.
...
- **Component 3 (Inline Editing & Feedback Loop)**:
  ...
  - Protected-value failures retain available draft panes, show a not-for-patient-use banner and failed-token list, and display FAILED instead of a misleading passing verbatim percentage. These drafts have no successful-check snapshot. Rechecks compare source numeric values and occurrence counts, including source-only values; a new passing revision is required to clear failures.

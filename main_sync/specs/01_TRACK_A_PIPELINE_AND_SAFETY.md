...
- `compose_clinical_text` binds template placeholders and appends the populated instruction-bearing fields in the effective `ClinicalOrders`: diagnosis, medications and instructions, hydration, fever thresholds, red flags, contraindications, and contacts. Patient name/MRN, age, and weight remain available in the source record and UI but are omitted from simplification and judge context. It does not invent clinical instructions.
- The UI passes an unchanged deep copy of the Team7 source order into generation. `InstructionPacket.source_clinical_orders` retains an independent immutable source snapshot and `InstructionPacket.clinical_orders` records the identical values actually sent through generation and safety checks. Physicians edit only the generated simplified text; the UI exposes no clinical-order override path.
...
- Protect doses/concentrations, temperatures, phone numbers including 911, timing/frequency, percentages, and instruction-level clinical measurements. Do not protect patient name/MRN, age, weight, list numbering, dates, or protocol versions. Demographic values stay in the source record and UI; age and weight are not included in the structured orders sent to the models, and their absence from simplified instructions does not fail protection or verbatim checks.
